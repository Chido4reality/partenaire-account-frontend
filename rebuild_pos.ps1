...
const PAYMENT_MODES = [
  ...
  { key: "partial", en: "Partial payment", fr: "Paiement partiel",  color: "#f59e0b" },
...
const PAY_METHODS = [
  ...
  { key: "bank",         en: "Bank transfer", fr: "Virement" },
];
...
export default function POSPage() {
  ...
  const { data: products } = useQuery({
    ...
    queryFn: () => search.length >= 2
      ? api.get(`/products?search=${search}&location_id=${selectedLocation?.id || ""}`).then(r => r.data)
      : { data: [] },
  ...
  const { data: customers } = useQuery({
    ...
    queryFn: () => custSearch.length >= 2
      ? api.get(`/customers?search=${custSearch}&limit=8`).then(r => r.data)
      : { data: [] },
  ...
  const addToCart = (product, qty = 1) => {
    setCart(prev => {
      ...
      return [...prev, {
        ...
        barcode: product.barcode, quantity: qty,
        unit_price: product.sell_price, cost_price: product.cost_price,
  ...
  const saleMutation = useMutation({
    mutationFn: () => api.post("/sales", {
      ...
      payment_method: payMethod, paid_amount: paid,
    ...
    onSuccess: () => {
      ...
      setPaidAmt(""); setDueDate(""); setNotes("");
  ...
  return (
    <div style={{ display: "flex", height: "100%" }}>
      {/* LEFT */}
      <div style={{ flex: 1, padding: 20, borderRight: "1px solid var(--border)", overflowY: "auto", minWidth: 0 }}>
        ...
        <div style={{ marginBottom: 12 }}>
          <label className="label">{lang === "en" ? "Selling from" : "Vendre depuis"}</label>
          <select className="input" value={selectedLocation?.id || ""}
            onChange={e => {
              const loc = locations.find(l => l.id === e.target.value);
              setLocation(loc || null);
            }}
            style={{ borderColor: !selectedLocation ? "#ef4444" : "var(--border)" }}>
          ...
          {!selectedLocation && (
            <div style={{ fontSize: 11, color: "#f87171", marginTop: 4 }}>
              {lang === "en" ? "Select a location to start selling" : "Choisir un emplacement pour vendre"}
            </div>
          )}
        </div>
        ...
        <div style={{ position: "relative", marginBottom: 12 }}>
          <input className="input" placeholder={lang === "en" ? "Search customer (optional)..." : "Chercher client (optionnel)..."}
            value={customer ? customer.name : custSearch}
          ...
          {customers?.data?.length > 0 && !customer && (
            <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 20, background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden", marginTop: 4, boxShadow: "0 8px 24px rgba(0,0,0,0.4)" }}>
              {customers.data.map(c => (
                <div key={c.id} onClick={() => { setCustomer(c); setCustSearch(""); }}
                  style={{ padding: "10px 14px", cursor: "pointer", borderBottom: "1px solid var(--border)", fontSize: 13 }}
                  onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.05)"}
                  onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                  <strong>{c.name}</strong>
                  {c.phone && <span style={{ color: "var(--text-muted)", marginLeft: 8 }}>{c.phone}</span>}
          ...
          {customer && (
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 6 }}>
              <span style={{ background: "rgba(79,70,229,0.15)", color: "var(--brand-light)", padding: "3px 10px", borderRadius: 20, fontSize: 12 }}>
        ...
        {/* Search */}
        <input ref={searchRef} className="input" placeholder={lang === "en" ? "Scan barcode or search product..." : "Scanner ou rechercher..."}
          value={search} onChange={e => setSearch(e.target.value)} style={{ marginBottom: 8 }} />

        <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 16 }}>
          {lang === "en" ? "USB barcode scanner ready - just scan" : "Lecteur USB pret - scannez directement"}
        </div>
          ...
          <div style={{ border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
            {products.data.map(p => (
              <div key={p.id} onClick={() => addToCart(p)}
                style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", cursor: "pointer", borderBottom: "1px solid var(--border)", transition: "background 0.1s" }}
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>
                    {p.barcode && p.barcode + " - "}
                    {p.pa_categories?.name}
                    {p.stock != null && " - Stock: " + p.stock.quantity + " " + p.unit}
                  </div>
                </div>
      ...
      {/* RIGHT: Cart */}
      <div style={{ width: 320, display: "flex", flexDirection: "column", background: "var(--bg-surface)" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontWeight: 600 }}>
            {lang === "en" ? "Cart" : "Panier"}
            {cart.length > 0 && <span style={{ color: "var(--brand-light)", marginLeft: 6 }}>({cart.length})</span>}
          </span>
          {cart.length > 0 && (
            <button onClick={() => setCart([])} style={{ background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", fontSize: 12 }}>
              {lang === "en" ? "Clear" : "Vider"}
            </button>
          )}
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {cart.length === 0 ? (
            <div style={{ textAlign: "center", padding: 32, color: "var(--text-muted)", fontSize: 12 }}>
              {lang === "en" ? "Cart is empty - scan or search a product" : "Le panier est vide"}
            </div>
          ) : cart.map((item, idx) => (
            <div key={idx} style={{ padding: "12px 16px", borderBottom: "1px solid var(--border)" }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <button onClick={() => updateQty(idx, item.quantity - 1)} style={{ width: 24, height: 24, borderRadius: 6, border: "1px solid var(--border)", background: "var(--bg-elevated)", color: "var(--text-primary)", cursor: "pointer" }}>-</button>
                <input type="number" value={item.quantity} min={1}
                  onChange={e => updateQty(idx, +e.target.value)}
                  style={{ width: 40, textAlign: "center", background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: 6, color: "var(--text-primary)", padding: "2px 4px", fontSize: 13 }} />
                <button onClick={() => updateQty(idx, item.quantity + 1)} style={{ width: 24, height: 24, borderRadius: 6, border: "1px solid var(--border)", background: "var(--bg-elevated)", color: "var(--text-primary)", cursor: "pointer" }}>+</button>
                <input type="number" value={item.unit_price} onChange={e => updatePrice(idx, e.target.value)}
                  style={{ flex: 1, background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: 6, color: "var(--text-primary)", padding: "4px 8px", fontSize: 12 }} />
                <div style={{ fontSize: 12, fontWeight: 600, color: "var(--brand-light)", minWidth: 68, textAlign: "right" }}>
        ...
        <div style={{ padding: 16, borderTop: "1px solid var(--border)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
            <span style={{ fontWeight: 700, fontSize: 16 }}>{lang === "en" ? "Total" : "Total"}</span>
            <span style={{ fontWeight: 800, fontSize: 20, color: "var(--brand-light)" }}>{formatCFA(total)}</span>
          </div>

          {!showPayment ? (
            <button className="btn btn-primary btn-block" disabled={cart.length === 0 || !selectedLocation}
              onClick={() => setShowPayment(true)}>
              {!selectedLocation
                ? (lang === "en" ? "Select a location first" : "Choisir un emplacement d abord")
                : (lang === "en" ? "Proceed to payment" : "Proceder au paiement")}
            </button>
          ) : (
            <div>
              <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
                {PAYMENT_MODES.map(pm => (
                  <button key={pm.key} onClick={() => setPayMode(pm.key)} style={{
                    ...
                    border: "1.5px solid " + (payMode === pm.key ? pm.color : "var(--border)"),
                    background: payMode === pm.key ? pm.color + "20" : "transparent",
                    color: payMode === pm.key ? pm.color : "var(--text-secondary)",
              ...
              {payMode === "partial" && (
                <input className="input" type="number" placeholder={lang === "en" ? "Amount paid" : "Montant paye"}
                  value={paidAmt} onChange={e => setPaidAmt(e.target.value)} style={{ marginBottom: 8 }} />
              )}

              {(payMode === "partial" || payMode === "credit") && (
                <input className="input" type="date" value={dueDate}
                  onChange={e => setDueDate(e.target.value)} style={{ marginBottom: 8 }} />
              )}
              ...
              <div style={{ background: "var(--bg-elevated)", borderRadius: 10, padding: "10px 12px", marginBottom: 10, fontSize: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  <span style={{ color: "var(--text-secondary)" }}>{lang === "en" ? "Total" : "Total"}</span>
                  <strong>{formatCFA(total)}</strong>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: paid < total ? 4 : 0 }}>
                  <span style={{ color: "#10b981" }}>{lang === "en" ? "Paid" : "Paye"}</span>
                  <strong style={{ color: "#10b981" }}>{formatCFA(paid)}</strong>
                </div>
                {paid < total && (
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ color: "#ef4444" }}>{lang === "en" ? "Balance due" : "Reste"}</span>
                    <strong style={{ color: "#ef4444" }}>{formatCFA(total - paid)}</strong>
                  </div>
                )}
              </div>
...
Write-Host "POS rebuilt cleanly!" -ForegroundColor Green
